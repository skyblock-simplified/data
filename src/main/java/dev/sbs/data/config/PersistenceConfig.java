...
/**
 * Wires the SkyBlock persistence layer for {@code data} against the dockerized
 ...
 * {@link dev.sbs.data.poller.AssetPoller}.
 *
 ...
 * session inside {@code data} and lets {@code AssetPoller} write to it via
 * {@link Qualifier @Qualifier("assetSession")}.
 *
 * <p>This class also exposes two bridge beans that decouple
 * {@link dev.sbs.data.poller.AssetPoller} from the {@code final}
...
public class PersistenceConfig {
     ...
     * {@link dev.sbs.data.write.WriteQueueConsumer} and
     * {@link dev.sbs.data.write.WriteBatchScheduler} can inject the
    ...
    public @NotNull JpaSession assetSession() {
        ...
        JpaSession session = new SessionManager().connect(config);
        log.info("data asset-state session wired with JpaCacheProvider.EHCACHE (schema=asset_state)");
        return session;
    }
    ...
    void logCacheProvider() {
        log.info("data persistence wired with JpaCacheProvider.HAZELCAST_CLIENT - "
